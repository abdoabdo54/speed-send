...
async def create_campaign(
    ...
    try:
        ...
        # IMMEDIATELY PREPARE AND SEND ALL EMAILS
        try:
            from app.google_api import GoogleWorkspaceService
            from app.encryption import encryption_service
            from app.models import EmailLog, EmailStatus, WorkspaceUser
            import logging
            
            logger = logging.getLogger(__name__)
            logger.info(f"🚀 IMMEDIATE SENDING: Campaign {new_campaign.id}")
            
            # Get all sender accounts with their users
            sender_accounts = new_campaign.sender_accounts
            ...
            email_logs = []
            for idx, recipient in enumerate(new_campaign.recipients):
            ...
            new_campaign.status = CampaignStatus.SENDING
            new_campaign.started_at = datetime.utcnow()
            db.commit()
            ...
            for idx, email_log in enumerate(email_logs):
                try:
                    ...
                    message_id = google_service.send_email(
                        ...
                        body_plain=new_campaign.body_plain,
                        custom_headers=new_campaign.custom_headers,
                        attachments=new_campaign.attachments
                    )
            ...
            new_campaign.failed_count = failed_count
            new_campaign.status = CampaignStatus.COMPLETED
            new_campaign.completed_at = datetime.utcnow()
            db.commit()
            
            logger.info(f"🎉 Campaign {new_campaign.id} completed: {sent_count} sent, {failed_count} failed")
            
        except Exception as e:
            logger.error(f"❌ Campaign {new_campaign.id} failed: {e}")
            new_campaign.status = CampaignStatus.FAILED
            db.commit()
